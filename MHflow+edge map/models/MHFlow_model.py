...
class MHFlowModel(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        if weight_fl > 0:
            ...
            z, nll, y_logits = self.netG(gt=self.real_H, lr=self.var_L, em=calculate_batch_edge_map(self.var_L), reverse=False,
                                         epses=epses,
                                         add_gt_noise=True)
        ...
        if weight_l1 > 0:
            ...
            sr, logdet = self.netG(lr=self.var_L, z=z.to(self.var_L.device), eps_std=0, em=calculate_batch_edge_map(self.var_L),
                                   reverse=True,
                                   add_gt_noise=True)
        ...
        if self.heats is not None:
                ...
                for i in range(self.n_sample):
                    ...
                    with torch.no_grad():
                        self.fake_H[(heat, i)], logdet = self.netG(lr=self.var_L, z=z,
                                                                   eps_std=heat, reverse=True, em=calculate_batch_edge_map(self.var_L),
                                                                   add_gt_noise=True)
        else:
            ...
            with torch.no_grad():
                ...
                self.fake_H[(0, 0)], logdet = self.netG(lr=self.var_L, z=z.to(self.var_L.device),
                                                        ...
                                                        em=calculate_batch_edge_map(self.var_L),
                                                        add_gt_noise=True)
        self.netG.train()
        return None
    ...
    def get_encode_nll(self, lq, gt):
        self.netG.eval()
        with torch.no_grad():
            _, nll, _ = self.netG(gt=gt, lr=lq, reverse=False, em=calculate_batch_edge_map(lq))
        self.netG.train()
    ...
    def get_encode_z(self, lq, gt, epses=None, add_gt_noise=True):
        self.netG.eval()
        with torch.no_grad():
            z, _, _ = self.netG(gt=gt, lr=lq, reverse=False, epses=epses, add_gt_noise=add_gt_noise, em=calculate_batch_edge_map(lq))
        self.netG.train()
        return z
    ...
    def get_encode_z_and_nll(self, lq, gt, epses=None, add_gt_noise=True):
        self.netG.eval()
        with torch.no_grad():
            z, nll, _ = self.netG(gt=gt, lr=lq, reverse=False, epses=epses, add_gt_noise=add_gt_noise, em=calculate_batch_edge_map(lq))
        self.netG.train()
        return z, nll
    ...
    def get_sr_with_z(self, lq, heat=None, seed=None, z=None, epses=None):
        ...
        with torch.no_grad():
            sr, logdet = self.netG(lr=lq, z=z.cuda(), eps_std=heat, reverse=True, add_gt_noise=True,em=calculate_batch_edge_map(lq))
        self.netG.train()
        return sr, z
...
def calculate_batch_edge_map(var_L):
    if var_L.dim() == 4:
    ...
    for i in range(batch_size):
        em = calculate_edge_map(var_L[i], device=device)
        batch_em.append(em[0])

    return torch.stack(batch_em)

def calculate_edge_map(image_tensor, device=None):
    """
    Calculate an edge map using Sobel and Laplacian filters.

    Parameters:
        image_tensor (torch.Tensor): A 4D tensor [1, C, H, W] or [C, H, W]

    Returns:
        torch.Tensor: Edge map tensor [1, H, W] (uint8)
    """
    if image_tensor.dim() == 3:
        image_tensor = image_tensor.unsqueeze(0)  # shape: [1, C, H, W]
    elif image_tensor.dim() != 4:
        raise NotImplementedError("Expected tensor of shape [C, H, W] or [B, C, H, W] with B=1")

    # Remove batch dimension
    image = image_tensor[0]  # shape: [C, H, W]

    # Convert to numpy and transpose to HWC
    image_np = image.detach().cpu().numpy()           # shape: [C, H, W]
    image_np = np.transpose(image_np, (1, 2, 0))       # shape: [H, W, C]
    image_np = (image_np * 255).clip(0, 255).astype(np.uint8)

    # Convert to grayscale
    gray = cv2.cvtColor(image_np, cv2.COLOR_BGR2GRAY)  # shape: [H, W]
    ...
    edge_map = cv2.bitwise_or(sobelCombined, lap)  # shape: [H, W]

    # Convert back to torch tensor with batch and channel dimensions
    edge_tensor = torch.from_numpy(edge_map).unsqueeze(0).unsqueeze(0).float() / 255.0  # shape: [1, 1, H, W]
