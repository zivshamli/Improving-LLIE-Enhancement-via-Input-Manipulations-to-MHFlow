...
class MHFlow(nn.Module):
    ...
    def forward(self, gt=None, lr=None, z=None, eps_std=None, reverse=False, epses=None,
                ...
                lr_enc=None,
                add_gt_noise=True, step=None, y_label=None, align_condition_feature=False, get_color_map=False, em: torch.Tensor=None):
        if get_color_map:
        ...
        if not reverse:
            ...
            return self.normal_flow(gt, lr, epses=epses, lr_enc=lr_enc, add_gt_noise=add_gt_noise, step=step,
                                    y_onehot=y_label, align_condition_feature=align_condition_feature, em=em)
        else:
            # assert lr.shape[0] == 1
            ...
            if reverse_with_grad:
                return self.reverse_flow(lr, z, y_onehot=y_label, eps_std=eps_std, epses=epses, lr_enc=lr_enc,
                                         add_gt_noise=add_gt_noise, em=em)
            else:
                with torch.no_grad():
                    return self.reverse_flow(lr, z, y_onehot=y_label, eps_std=eps_std, epses=epses, lr_enc=lr_enc,
                                             add_gt_noise=add_gt_noise, em=em)
    ...
    def normal_flow(self, gt, lr, y_onehot=None, epses=None, lr_enc=None, add_gt_noise=True, step=None,
                    align_condition_feature=False, em=None):
        
        if self.opt['to_yuv']:
            gt = self.rgb2yuv(gt)
        if lr_enc is None and self.RRDB:
            lr_enc = self.rrdbPreprocessing(lr, em)
        ...
        if align_condition_feature:
            with torch.no_grad():
                gt_enc = self.rrdbPreprocessing(gt, em)
    ...
    def rrdbPreprocessing(self, lr, em: torch.Tensor = None):
        rrdbResults = self.RRDB(lr, em)
    ...
    def reverse_flow(self, lr, z, y_onehot, eps_std, epses=None, lr_enc=None, add_gt_noise=True, em:torch.Tensor = None):
