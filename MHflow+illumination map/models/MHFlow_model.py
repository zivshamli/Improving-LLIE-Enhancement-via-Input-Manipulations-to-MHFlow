...
class MHFlowModel(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        if weight_fl > 0:
            ...
            z, nll, y_logits = self.netG(gt=self.real_H, lr=self.var_L, illum_map=calculate_batch_illum_map(self.var_L), reverse=False,
                                         epses=epses,
                                         add_gt_noise=True)
        ...
        if weight_l1 > 0:
            ...
            sr, logdet = self.netG(lr=self.var_L, z=z.to(self.var_L.device), eps_std=0, illum_map=calculate_batch_illum_map(self.var_L),
                                   reverse=True,
                                   add_gt_noise=True)
        ...
        if self.heats is not None:
                ...
                for i in range(self.n_sample):
                    ...
                    with torch.no_grad():
                        self.fake_H[(heat, i)], logdet = self.netG(lr=self.var_L, z=z,
                                                                   eps_std=heat, reverse=True, illum_map=calculate_batch_illum_map(self.var_L),
                                                                   add_gt_noise=True)
        else:
            ...
            with torch.no_grad():
                ...
                self.fake_H[(0, 0)], logdet = self.netG(lr=self.var_L, z=z.to(self.var_L.device),
                                                        ...
                                                        illum_map=calculate_batch_illum_map(self.var_L),
                                                        add_gt_noise=True)
        self.netG.train()
        return None
    ...
    def get_encode_nll(self, lq, gt):
        self.netG.eval()
        with torch.no_grad():
            _, nll, _ = self.netG(gt=gt, lr=lq, reverse=False, illum_map=calculate_batch_illum_map(lq))
        self.netG.train()
    ...
    def get_encode_z(self, lq, gt, epses=None, add_gt_noise=True):
        self.netG.eval()
        with torch.no_grad():
            z, _, _ = self.netG(gt=gt, lr=lq, reverse=False, epses=epses, add_gt_noise=add_gt_noise, illum_map=calculate_batch_illum_map(lq))
        self.netG.train()
        return z
    ...
    def get_encode_z_and_nll(self, lq, gt, epses=None, add_gt_noise=True):
        self.netG.eval()
        with torch.no_grad():
            z, nll, _ = self.netG(gt=gt, lr=lq, reverse=False, epses=epses, add_gt_noise=add_gt_noise, illum_map=calculate_batch_illum_map(lq))
        self.netG.train()
        return z, nll
    ...
    def get_sr_with_z(self, lq, heat=None, seed=None, z=None, epses=None):
        ...
        with torch.no_grad():
            sr, logdet = self.netG(lr=lq, z=z.cuda(), eps_std=heat, reverse=True, add_gt_noise=True,illum_map=calculate_batch_illum_map(lq))
        self.netG.train()
        return sr, z
