   ...
   # def feed_data(self, data, need_GT=True):
     ...
     #       h, w = tensor.shape[-2:]
      #      pad_h = 0 if h % 2 == 0 else 1
       #     pad_w = 0 if w % 2 == 0 else 1
        #    return F.pad(tensor, (0, pad_w, 0, pad_h), mode='reflect')  # pad (left, right, top, bottom)

      #self.var_L = pad_to_even(data['LQ'].to(self.device))  # low-light input
      #self.zero_channel = pad_to_even(data['zero_img'].to(self.device))

      #if need_GT:
       #     self.real_H = pad_to_even(data['GT'].to(self.device))
    ...
    def optimize_parameters(self, step):
        ...
        if weight_fl > 0:
            ...
            z, nll, y_logits = self.netG(gt=self.real_H, lr=self.var_L, upm=calculate_batch_upm(self.var_L), reverse=False,
                                         epses=epses,
                                         add_gt_noise=True)
        ...
        if weight_l1 > 0:
            ...
            sr, logdet = self.netG(lr=self.var_L, z=z.to(self.var_L.device), eps_std=0, upm=calculate_batch_upm(self.var_L),
                                   reverse=True,
                                   add_gt_noise=True)
        ...
        if self.heats is not None:
                ...
                for i in range(self.n_sample):
                    ...
                    with torch.no_grad():
                        self.fake_H[(heat, i)], logdet = self.netG(lr=self.var_L, z=z,
                                                                   eps_std=heat, reverse=True, upm=calculate_batch_upm(self.var_L),
                                                                   add_gt_noise=True)
        else:
            ...
            with torch.no_grad():
                ...
                self.fake_H[(0, 0)], logdet = self.netG(lr=self.var_L, z=z.to(self.var_L.device),
                                                        ...
                                                        upm=calculate_batch_upm(self.var_L),
                                                        add_gt_noise=True)
        self.netG.train()
        return None
    ...
    def get_encode_nll(self, lq, gt):
        self.netG.eval()
        with torch.no_grad():
            _, nll, _ = self.netG(gt=gt, lr=lq, reverse=False, upm=calculate_batch_upm(lq))
        self.netG.train()
    ...
    def get_encode_z(self, lq, gt, epses=None, add_gt_noise=True):
        self.netG.eval()
        with torch.no_grad():
            z, _, _ = self.netG(gt=gt, lr=lq, reverse=False, epses=epses, add_gt_noise=add_gt_noise, upm=calculate_batch_upm(lq))
        self.netG.train()
        return z
    ...
    def get_encode_z_and_nll(self, lq, gt, epses=None, add_gt_noise=True):
        self.netG.eval()
        with torch.no_grad():
            z, nll, _ = self.netG(gt=gt, lr=lq, reverse=False, epses=epses, add_gt_noise=add_gt_noise, upm=calculate_batch_upm(lq))
        self.netG.train()
        return z, nll
    ...
    def get_sr_with_z(self, lq, heat=None, seed=None, z=None, epses=None):
        ...
        with torch.no_grad():
            sr, logdet = self.netG(lr=lq, z=z.cuda(), eps_std=heat, reverse=True, add_gt_noise=True,upm=calculate_batch_upm(lq))
        self.netG.train()
        return sr, z
...
def calculate_batch_upm(var_L):
    ...
    for i in range(batch_size):
        upm = calculate_upm(var_L[i])
        batch_upm.append(upm)

    return torch.stack(batch_upm)

def calculate_upm(var_L):
    dark = var_L
    R_split, G_split, B_split = torch.split(dark, 1)
